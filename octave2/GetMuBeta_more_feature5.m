function [mu,Beta] = GetMuBeta_more_feature5(start, endD, data,HDR)
...
BETA					  =  abs(fft(Data_be));
		[mu(k,f) xtemp]   	 	=  min(Mu) ;
		[mu(k,f+1) xtemp]    	=  max(Mu) ;
		mu(k,f+2) = mean(Mu);
		[Beta(k,f) ytemp]  	=  max(BETA) ; 
		[Beta(k,f+1) ytemp]  	=  min(BETA) ; 
		Beta(k,f+2) = mean(BETA);   
			else
