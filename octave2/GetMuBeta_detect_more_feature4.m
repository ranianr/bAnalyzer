function [mu,Beta] = GetMuBeta_detect_more_feature4(data)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	mu   = zeros(1,nChannels*4);
	Beta = zeros(1,nChannels*4);
	...
	for f = 1:4:nChannels*4
		...
		[mu(1,f+2) mu(1,f+3)]    	=  max(Mu) ;
		[Beta(1,f) Beta(1,f+1)]  	=  max(BETA) ; %eshm3na el mean leh mesh el RMS mathlan ?! 
		[Beta(1,f+2) Beta(1,f+3)]  	=  min(BETA) ; %eshm3na el mean leh mesh el RMS mathlan ?! 
	end
end
